...
knitr::opts_knit$set(root.dir = "~/Users/Jules/Documents/UMich_Bio201_F19/Lab3/")
```
...
nrow(scfa_indv) #Number of rows, 4078
ncol(scfa_indv) #Number of columns/variables, 19
dim(scfa_indv) #Number of rows and columns
colnames(scfa_indv) #Names of columns, including Participant_ID, Sex, Age...
rownames(scfa_indv) #Numbered rows, printed 1000 and omitted 3078
glimpse(scfa_indv) #Overview of the data, including observations, columns, class, and preview of the data points per row
str(scfa_indv) #Structure, which shows class and length for 19 variables with preview of values
```
...
#Measured after sample was added
```
...
```{r}
scfa_indv <- rename_all(scfa_indv, tolower) #snake case
...
```{r}
select(scfa_indv, study_week, participant_id, butyrate_mM) #include these columns only
```
...
```{r}
select(scfa_indv, -use_data, -notes) #exclude these columns
```
...
```{r}
filter(scfa_indv, semester == "Fall2018") #include matches for this value
```
...
f18_but_rps <- scfa_indv %>%
  ...
  filter(notes != "frozen >24hrs") %>% 
  filter(sample_wt_g > 0.10) %>% 
  select("participant_id", "butyrate_mM", "sample_wt_g")
...
```{r}
mutate(scfa_indv, sample_kg = sample_wt_g / 1000)  #new column for kg sample measurements
```
...
scfa_indv_qc <- scfa_indv %>%
  ...
  select(-sample_wt_g) %>% 
  mutate(acetate_mmol_kg = round((acetate_mm / sample_wt_kg), digits = 2),
        butyrate_mmol_kg = round((butyrate_mm / sample_wt_kg), digits =2),
        propionate_mmol_kg = round((propionate_mm / sample_wt_kg), digits = 2),
        total_scfa_mmol_kg = round((acetate_mmol_kg + butyrate_mmol_kg + propionate_mmol_kg), digits = 2))
