...
colnames(scfa_long)
# participant_id, study_week, semester
```
...
write_delim(tuk_wk1, path = "C:/Users/Jules/Documents/UMich_Bio201_F19/Lab6/curated_data/tuk_wk1.txt",
...
```{r}
# import methane data 
breath_wkly <- read_delim("C:/Users/Jules/Documents/UMich_Bio201_F19/Lab5/curated_data/breath_wkly.txt", 
                        delim = "\t", col_names = TRUE, trim_ws = TRUE, na = c("NA")) %>%
# format methane data
  rename_all(tolower) %>%
...
# run the test
kruskal.test(supplement_consumed ~ ch4_mean, breath_wkly) #p-value: 0.1579
```
...
```{r}
# hypothesis: 
#Null: hydrogen breath levels do not differ between participants consuming fiber supplements.
#Alternative: hydrogen breath levels do differ between participants consuming fiber supplements.

# plot 
hydrogen_violin_plot <- breath_wkly %>% 
   ggplot(aes(x = supplement_consumed, 
             y = h2_mean, 
             color = supplement_consumed)) + 
  geom_violin() + 
  ...
  theme(legend.position = "none",
        axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
save_plot(filename = "C:/Users/Jules/Documents/UMich_Bio201_F19/Lab6/figures/hydrogen_violin_plot.pdf",
          plot = hydrogen_violin_plot ,
          nrow = 1, ncol = 1,
...
```{r}
# conclusion 
#With a p-value of 0.3001, we fail to reject the null hypothesis that hydrogen breath levels do not differ between participants consuming fiber supplements.
```
...
```{r}
# plot
firmicutes_butyrate_plot <- tax_ratio %>%
  ggplot(aes(x = butyrate_mean,
             y = firmicutes_abund)) + 
  geom_point() + 
  ...
  labs(title = "Firmicutes abundance vs. butyrate concentration")
firmicutes_butyrate_plot

save_plot(filename = "C:/Users/Jules/Documents/UMich_Bio201_F19/Lab6/figures/firmicutes_butyrate_plot.pdf",
          plot = firmicutes_butyrate_plot,
          nrow = 1, ncol = 1, 
          base_aspect_ratio = 1.1)
...
bratio_fbratio_plot <- tax_ratio %>%
  ...
  labs(title = "Butyrate ratio vs. Firmicutes to Bacteroidetes ratio")
bratio_fbratio_plot

save_plot(filename = "C:/Users/Jules/Documents/UMich_Bio201_F19/Lab6/figures/bratio_fbratio_plot.pdf",
          plot = bratio_fbratio_plot,
          nrow = 1, ncol = 1, 
          base_aspect_ratio = 1.1)
...
#Yes, we draw the same conclusions.
```
...
```{r}
bacteroidetes_propionate_plot <- tax_ratio %>%
  ...
  labs(title = "Bacteroides abundance vs. propionate concentration")
bacteroidetes_propionate_plot

save_plot(filename = "C:/Users/Jules/Documents/UMich_Bio201_F19/Lab6/figures/bacteroidetes_propionate_plot.pdf",
          plot = bacteroidetes_propionate_plot,
          nrow = 1, ncol = 1, 
          base_aspect_ratio = 1.1)
...
pratio_fbratio_plot <- tax_ratio %>%
  ...
  ylab("Firmicutes to Bacteroidetes ratio") +
  labs(title = "Propionate ratio vs. Firmicutes to Bacteroidetes ratio")
pratio_fbratio_plot

save_plot(filename = "C:/Users/Jules/Documents/UMich_Bio201_F19/Lab6/figures/pratio_fbratio_plot.pdf",
          plot = pratio_fbratio_plot,
          nrow = 1, ncol = 1, 
          base_aspect_ratio = 1.1)
...
```{r}
# import 
pH_BSS_wkly <- read_delim(file = "C:/Users/Jules/Documents/UMich_Bio201_F19/Lab4/curated_data/pH_BSS_wkly.txt",
...
```{r}
scfa_acetate_plot <- tax_scfa_pH %>%
  ggplot(aes(x = acetate_mean,
             y = relative_abundance)) + 
  geom_point() + 
  geom_smooth(method = "lm",
              se = FALSE) +  
  xlab("Acetate (mmol/kg)") + 
  ylab("Total SCFA abundance") +
  labs(title = "Total SCFAs vs. Acetate")
scfa_acetate_plot

save_plot(filename = "C:/Users/Jules/Documents/UMich_Bio201_F19/Lab6/figures/scfa_acetate_plot.pdf",
...
scfa_aratio_plot <- tax_scfa_pH %>%
  ...
  xlab("Acetate ratio") + 
  ylab("Total SCFA abundance") +
  labs(title = "SCFA abundance vs. acetate ratio")
scfa_aratio_plot

save_plot(filename = "C:/Users/Jules/Documents/UMich_Bio201_F19/Lab6/figures/scfa_aratio_plot.pdf",
          plot = scfa_aratio_plot,
          nrow = 1, ncol = 1, 
          base_aspect_ratio = 1.1)
...
summary(m65b) 
#Adjusted R-squared: 0.0001098
#p-value: 0.1988

#Different conclusions - linear model has slight negative slope, much lower p-value
```
...
scfa_ph_plot <- tax_scfa_pH %>%
  ...
  xlab("pH") + 
  ylab("Total SCFA abundance") +
  labs(title = "Total SCFAs vs. pH")
scfa_ph_plot

save_plot(filename = "C:/Users/Jules/Documents/UMich_Bio201_F19/Lab6/figures/scfa_ph_plot.pdf",
          plot = scfa_ph_plot,
          nrow = 1, ncol = 1, 
          base_aspect_ratio = 1.1)
...
# correlation test 
m66 <- tax_scfa_pH %>%
  lm(relative_abundance ~ mean_ph, data = .)
summary(m66) 
#Adjusted R-squared: -2.266e-05
#p-value: 0.3522

#There doesn't appear to be a linear relationship between total SCFAs and pH.
```
...
butyrate_ph_plot <- tax_scfa_pH %>%
  ...
  xlab("pH") + 
  ylab("Butyrate") +
  labs(title = "Butyrate vs. pH")
butyrate_ph_plot

save_plot(filename = "C:/Users/Jules/Documents/UMich_Bio201_F19/Lab6/figures/butyrate_ph_plot.pdf",
...
# correlation test 
m67 <- tax_scfa_pH %>%
  lm(butyrate_mean ~ mean_ph, data = .)
summary(m67) 
#Adjusted R-squared: 0.05209
#p-value: 2.2e-16

#There appears to be a very weak negative linear relationship, if any.
```
