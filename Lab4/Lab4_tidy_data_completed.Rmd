...
```{r}
scfa_indv_data <- read_delim(file = "/Users/Jules/Documents/UMich_Bio201_F19/Lab3/curated_data/scfa_indv_qc.txt",
...
```{r}
# write code here
scfa_indv_summary1 <- scfa_indv_data %>% 
...
acetate_wide <- scfa_wkly_qc %>%
  select(participant_id, semester, supplement_consumed, study_week, acetate_mean_mmol_kg) %>% 
  filter(study_week == "week1" | study_week == "week3") %>% 
  spread(study_week, acetate_mean_mmol_kg) %>% 
  na.omit(week1, week3) %>% 
...
propionate_wide <- scfa_wkly_qc %>%
  select(participant_id, semester, supplement_consumed, study_week, propionate_mean_mmol_kg) %>% 
  filter(study_week == "week1" | study_week == "week3") %>% 
  spread(study_week, propionate_mean_mmol_kg) %>% 
  na.omit(week1, week3) %>% 
...
```{r}
# write code here
butyrate_wide_delta <- butyrate_wide %>% 
...
```{r}
# copy+paste and edit code chunk for acetate
acetate_wide_delta <- acetate_wide %>% 
  group_by(supplement_consumed) %>% 
...
```{r}
# copy+paste and edit code chunk for propionate
propionate_wide_delta <- propionate_wide %>% 
  group_by(supplement_consumed) %>% 
...
acetate_plot_1 <- acetate_wide_delta %>%
    ggplot(aes(x = supplement_consumed, 
             y = mean_delta_acetate)) +
  geom_point(aes(color = supplement_consumed)) +
  labs(title = "Mean acetate concentration",
       x = "Supplement consumed",
       y = "Mean change in acetate (mmol/kg)") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1), legend.position = "none")

acetate_plot_1

butyrate_plot_1 <- butyrate_wide_delta %>%
    ggplot(aes(x = supplement_consumed, 
             y = mean_delta_butyrate)) +
  geom_point(aes(color = supplement_consumed), shape = "triangle") +
  labs(title = "Mean butyrate concentration",
       x = "Supplement consumed",
       y = "Mean change in butyrate (mmol/kg)") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1), legend.position = "none")
    
butyrate_plot_1

propionate_plot_1 <- propionate_wide_delta %>%
    ggplot(aes(x = supplement_consumed, 
             y = mean_delta_propionate)) +
  geom_point(aes(color = supplement_consumed), shape = "square") +
  labs(title = "Mean propionate concentration",
       x = "Supplement consumed",
       y = "Mean change in propionate (mmol/kg)") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1), legend.position = "none")
...
scfa_change_plot <- total_scfas %>%
  ggplot(aes(x = participant_id, 
             y = delta_acetate)) +
  geom_point(aes(color = semester)) +
  facet_grid(~semester) +
  labs(title = "Mean change in SCFAs",
       x = "Individuals",
       y = "Change in SCFAs (mmol/kg)") +
...
scfa_change_plot <- (total_scfas <- inner_join(acetate_wide, butyrate_wide, 
                        by = c("participant_id", "semester", "supplement_consumed")) %>% 
  full_join(propionate_wide,
            by = c("participant_id", "semester", "supplement_consumed")) %>% 
  mutate(total_wk1 = ace_wk1 + but_wk1 + pro_wk1,
...
wkly_means <- scfa_long %>%
  ...
  # mean per every group (by mean and scfa)
...
```{r}
write_delim(total_scfas, path = "C:/Users/Jules/Documents/UMich_Bio201_F19/Lab4/curated_data/total_scfas_wide.txt", delim = "\t")
```

Save the other data frames and plots generated today. Be sure they are placed in the appropriate directories (folders):
```{r}
# for data frames:
write_delim(scfa_indv_summary1, path = "C:/Users/Jules/Documents/UMich_Bio201_F19/Lab4/curated_data/scfa_indv_summary1.txt", delim = "\t")
write_delim(acetate_wide, path = "C:/Users/Jules/Documents/UMich_Bio201_F19/Lab4/curated_data/acetate_wide.txt", delim = "\t")
write_delim(butyrate_wide, path = "C:/Users/Jules/Documents/UMich_Bio201_F19/Lab4/curated_data/butyrate_wide.txt", delim = "\t")
write_delim(propionate_wide, path = "C:/Users/Jules/Documents/UMich_Bio201_F19/Lab4/curated_data/propionate_wide.txt", delim = "\t")
write_delim(wkly_means, path = "C:/Users/Jules/Documents/UMich_Bio201_F19/Lab4/curated_data/wkly_means.txt", delim = "\t")

# for plots:
#complete
save_plot(filename = "propionate_plot_1.pdf",
          plot = propionate_plot_1,
          nrow = 1, ncol = 1, 
          base_aspect_ratio = 1.1)
save_plot(filename = "acetate_plot_1.pdf",
          plot = acetate_plot_1,
          nrow = 1, ncol = 1, 
          base_aspect_ratio = 1.1)
save_plot(filename = "butyrate_plot_1.pdf",
          plot = butyrate_plot_1,
          nrow = 1, ncol = 1, 
          base_aspect_ratio = 1.1)
save_plot(filename = "scfa_change_plot.pdf",
          plot = scfa_change_plot,
          nrow = 1, ncol = 1)
save_plot(filename = "p1_plot.pdf",
          plot = p1,
          nrow = 1, ncol = 1)
...
```{r}
# import pH data
pH_data <- read_delim(file = "C:/Users/Jules/Documents/UMich_Bio201_F19/Lab4/raw_data/pH_indv.txt",
                        delim = "\t", col_names = TRUE, trim_ws = TRUE, 
                        na = c("", "NA")) %>%
  rename_all(tolower)

# filter to remove bad measurements based on outside list
pH_bad <- read_delim(file = "C:/Users/Jules/Documents/UMich_Bio201_F19/Lab4/raw_data/pH_bad.txt",
                        delim = "\t", col_names = TRUE, trim_ws = TRUE, 
...
pH_qc <- pH_data %>%
  ...
  group_by(participant_id, supplement_consumed, study_week) %>%
  summarise(mean_ph = mean(ph, na.rm = TRUE)) #calculate weekly average
dim(pH_qc)
```

```{r}
# import BSS
BSS_data <- read_delim(file = "C:/Users/Jules/Documents/UMich_Bio201_F19/Lab4/raw_data/BSS_indv.txt",
                        delim = "\t", col_names = TRUE, trim_ws = TRUE, 
                        na = c("", "NA")) %>%
  rename_all(tolower)
...
BSS_qc <- BSS_data %>%
  ...
  group_by(participant_id, supplement_consumed, study_week) %>%
  summarise(mean_bristol_score = mean(bristol_numeric, na.rm = TRUE)) #calculate weekly average 
dim(BSS_qc)
...
pH_BSS_wkly <- full_join(pH_qc, BSS_qc,
  ...
  mutate(mean_pH = round(mean_ph, digits = 2),
         mean_BSS = round(mean_bristol_score, digits = 2)) %>% #round avgs to reflect sig figs 
  select(-mean_ph, -mean_bristol_score)
```

To make sure that everyone has the same data set, check the number of rows and columns by typing dim().
```{r}
dim(pH_BSS_wkly)
#five columns including study_week
colnames(pH_BSS_wkly)
```
...
```{r}
write_delim(pH_BSS_wkly, path = "C:/Users/Jules/Documents/UMich_Bio201_F19/Lab4/curated_data/pH_BSS_wkly.txt",
