...
scfa_qc <- read_delim(file = "curated_data/SCFA_wkly_qc.txt", 
                        ...
                        col_types = list())
scfa_wkly_qc <- scfa_qc %>% 
  rename_all(tolower) %>% 
  select(-acetate_median, -butyrate_median, -propionate_median, -total_median) %>% 
  filter(frequency == "2xdaily") %>% 
  rename(acetate_mean_mmol_kg = acetate_mean, butyrate_mean_mmol_kg = butyrate_mean, propionate_mean_mmol_kg = propionate_mean, total_mean_mmol_kg = total_mean)
...
```{r}
ggplot(data = scfa_wkly_qc, #call dataset
       mapping = aes(x = participant_id, 
...
plot1 <- ggplot(data = scfa_wkly_qc, 
       mapping = aes(x = participant_id, 
                     y = butyrate_mean_mmol_kg))
# Draw the plot
plot1 + geom_point()
...
ggplot(data = scfa_wkly_qc, 
    ...
    theme(axis.text.x = element_blank())
```
 ggplot can't make sense of this data structure for a line plot. We have to modify the data assigned to each axis, and as mentioned in the error message above, we need to set a group. 
```{r}
ggplot(data = scfa_wkly_qc, 
...
```{r}
plot_ace <- ggplot(data = scfa_wkly_qc, 
       mapping = aes(x = participant_id, 
...
```{r}
plot_but <- ggplot(data = scfa_wkly_qc, 
       mapping = aes(x = participant_id, 
                     y = butyrate_mean_mmol_kg, 
                     color = study_week)) +
...
```{r}
plot_pro <- ggplot(data = scfa_wkly_qc, 
       mapping = aes(x = participant_id, 
...
```{r}
plot_tot <- ggplot(data = scfa_wkly_qc, 
       mapping = aes(x = participant_id, 
                     y = total_mean_mmol_kg, 
                     color = study_week)) +
...
plot_wk3 <- scfa_wkly_qc %>%
  filter(study_week == "week3") %>%
  ggplot(aes(x = participant_id, 
             y = total_mean_mmol_kg)) +
    geom_point(color = "mistyrose1") +
    labs(title = "Week 3",
         x = "Individuals",
         y = "Total fecal SCFAs (mmol/kg)") +
    theme(axis.text.x = element_blank(),
...
> ** enter answer here ** 
There is only one label for the x-axis in the facetted figure. The y-axis scale is the same for both panels.
...
Export plot 3.6 
```{r}
save_plot(filename = "Plot3_6.pdf",
          plot = plot3_6,
          nrow = 1, ncol = 1)
```
