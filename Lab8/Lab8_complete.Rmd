...
This step calculates the diversity index (using Bray Curtis in the example below), and does the ranking process of the diveristy indices to generate the information necessary to create a two-dimensional plot. Notice the arguments in the ordinate function; the index, Bray Curtis ("bray") or Jaccard ("jaccard"), is specified in the distance agurment, and with either of these metrics we will use method = "NMDS". The place were you might want to make some modifications is the k (this should be somewhere between 1-3), the maximum number of iterations (more than 100 will take several minutes to calcaulte), and the inital number of interations to try (50 in the example below). These can be modified to reduce the stress, with the goal of a number less than 0.20 (the lower the better). 
```{r}
...
physq_j <- ordinate(physq_sub, 
                      ...
                      distance = "jaccard")
physq_j #stress value: 0.225598 (Stress type 1, weak ties)
...
### Homework 8.5
Conduct an ADONIS test on a distance matrix calculated with the Jaccard index. Write your interpretations of the data below, be sure to explicitly state the calculated R- and p-values. 
```{r}
