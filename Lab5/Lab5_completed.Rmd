...
#much greater range in week3, which looks more unimodal and skews to the right compared to week1, which looks slightly bimodal
```
...
ch4_long %>%
  filter(study_week == "week1" | study_week == "week3") %>% 
  group_by(study_week) %>% 
  summarise(sample_size = n()) #168, 153 > 30. 

#normality test for week 1 ph samples
wk1_ch4 <- ch4_long %>%
  filter(study_week == "week1") 
shapiro.test(wk1_ch4$ch4_mean) #p-value: 2.2e-16
#visually checking normality
ch4_wk1_plot <- ggplot(wk1_ch4, aes(x = ch4_mean)) + geom_histogram()
ch4_wk1_plot #appears unimodal but skewed and noncontinuous
#saving plots:
...
var.test(x = week1_ch4_grp$ch4_mean, 
         ...
         alternative = "two.sided")
#p value < .05, so not equal variances
...
acetate_long %>%
  filter(study_week == "week1" | study_week == "week3") %>% 
  group_by(study_week) %>% 
  summarise(sample_size = n())  #146, 143 > 30 Given our large sample sizes, we can proceed with the paired t-test.
...
var.test(x = week1_ace_grp$acetate_mean, 
         ...
         alternative = "two.sided") #p-value is 0.00019 < .05, reject null hypothesis. not equal variances
...
propionate_long %>%
  filter(study_week == "week1" | study_week == "week3") %>% 
  group_by(study_week) %>% 
  summarise(sample_size = n()) #131, 139 > 30 Given our large sample sizes, we can proceed with the paired t-test.
...
var.test(x = week1_pro_grp$propionate_mean, 
         ...
         alternative = "two.sided") #p-value < .05, not equal variances
...
ph_long %>%
  filter(study_week == "week1" | study_week == "week3") %>% 
  group_by(study_week) %>% 
  summarise(sample_size = n()) #109, 138 > 30 Given our large sample sizes, we can proceed with the paired t-test.
...
var.test(x = week1_ph_grp$ph_mean, 
         ...
         alternative = "two.sided") # 0.0036 < 0.05, not equal variances
...
#A p-value = 0.002373 indicates that we should reject the null hypothesis - the wk1 and wk3 methane means are statistically different. 
```
...
t.test(x = acetate_df$acetate_mean_wk1, y = acetate_df$acetate_mean_wk3, 
       alternative = "less", paired = TRUE, var.equal = FALSE) 
#A p-value = 0.003459 indicates that we should reject the null hypothesis - the wk1 and wk3 acetate means are statistically different.
```
...
#A p-value = 0.3541 indicates that we fail to reject the null hypothesis - the wk1 and wk3 acetate means are statistically the same.
```
...
#A p-value = 0.01013 indicates that we fail to reject the null hypothesis - the wk1 and wk3 acetate means are statistically the same.
```
...
summary(aov_results) #F value of 0.661 - fail to reject null hypothesis is not likely to be false, no significant difference 
```
...
summary(acetate_aov_results) #We obtain a p-value of 0.137, which shows that the groups are not the same.
```
...
s2 <- scfa_wide %>%
  filter(supplement_consumed == "Arabino") 
shapiro.test(s2$delta_propionate) #only two samples. NA
...
summary(propionate_aov_results) #We obtain a p-value of 0.197, which shows that the groups are not the same.
```
